...
wxString g_strCAPEM = wxEmptyString;
...
extern "C"
{
    ...
    size_t wxcurl_string_write(void *ptr, size_t size, size_t nmemb, void *pcharbuf)
    {
        ...
        if (pStr)
        {
            wxString stmp = wxString((const char *)ptr, wxConvLibc, iRealSize); // Zend corrections for HTTP 1.1. chunk mode
            wxString str = wxCURL_BUF2STRING(*pStr) + stmp;
            *pStr = wxCURL_STRING2BUF(str);
        }
    ...
    /** call back function to retrieve CA */
    CURLcode sslctx_function(CURL *curl, void *sslctx, void *parm)
    {
        CURLcode rv = CURLE_ABORTED_BY_CALLBACK;
        if (g_strCAPEM.IsEmpty())
            return rv;

        BIO *cbio = BIO_new_mem_buf((const char *)g_strCAPEM.mb_str(), g_strCAPEM.Length());
...
void wxCurlBase::DumpErrorIfNeed(CURLcode error)
{
    ...
    const char *ch = curl_easy_strerror(error);
    m_szLastError = wxString::FromAscii(ch);
...
//Zend::
void wxCurlBase::SetFullURL(const wxString &szFullURL)
{
    m_szCurrFullURL = wxCURL_STRING2BUF(szFullURL);
}
...
void wxCurlBase::SetCurlHandleToDefaults(const wxString &relativeURL)
{
    ...
    if (m_pCURL /*&& ResetHandle() - cancells all options that were previously set*/)
    {
        ResetResponseVars();
...
bool wxCurlBase::SetCAInfo(const wxString &strPathToCAFile, const wxString &strPathToCAFolder)
{
    ...
    {
        res = curl_easy_setopt(m_pCURL, CURLOPT_CAINFO, NULL);
    }
    else
    {
    ...
    {
        res = curl_easy_setopt(m_pCURL, CURLOPT_CAPATH, NULL);
    }
    else
    {
        res = curl_easy_setopt(m_pCURL, CURLOPT_CAINFO, (const char *)strPathToCAFolder.mb_str());
    }
    return (res == CURLE_OK);
} //..SetCAInfo

bool wxCurlBase::SetCAPEM(const wxString &strCAPEM)
{
    g_strCAPEM = strCAPEM;
    // clear first
    SetCAInfo(wxEmptyString, wxEmptyString);
    // set updated
    CURLcode res = curl_easy_setopt(m_pCURL, CURLOPT_SSL_CTX_FUNCTION, sslctx_function);
    return (res == CURLE_OK);
} //..SetCAPEM
